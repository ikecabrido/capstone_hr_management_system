...
function generateChecklist(leaveType, leaveData, displayOnly = false) {
    ...
    // Get checklist configuration FROM lc_controller via AJAX
...
/**
 * Check if employee has paid leave days remaining
 * This is called automatically for non-maternity/paternity leave types
 */
function checkPaidLeaveDays(employeeId, leaveType, requestedDays, displayOnly = false) {
    // Skip for maternity and paternity leave
    if (leaveType === 'Maternity Leave' || leaveType === 'Paternity Leave') {
        return;
    }
    
    fetch('leave_management.php?action=check_paid_leave&employee_id=' + employeeId + '&leave_type=' + encodeURIComponent(leaveType) + '&requested_days=' + requestedDays)
        .then(response => response.json())
        .then(data => {
            const checkbox = document.getElementById('paid_leave_check');
            const statusEl = document.getElementById('status_paid_leave_check');
            
            if (checkbox && !displayOnly) {
                // Auto-check based on result
                checkbox.checked = data.has_paid_leave;
                ...
                if (label) {
                    ...
                    if (data.has_paid_leave) {
                        label.innerHTML += ' <span class="text-success"><i class="fas fa-check-circle"></i> ' + data.remaining_days + ' days remaining</span>';
                    } else {
                        label.innerHTML += ' <span class="text-danger"><i class="fas fa-times-circle"></i> ' + data.remaining_days + ' days remaining</span>';
                    }
                }
            } else if (statusEl && displayOnly) {
                // Display-only mode
                if (data.has_paid_leave) {
        ...
        .catch(error => {
            console.error('Error checking paid leave days:', error);
        });
}

/**
 * Check if leave was filed at least 30 days in advance
 * This is called automatically for Vacation Leave
 */
function checkFiledInAdvance(leaveId, displayOnly = false) {
    fetch('leave_management.php?action=check_filed_advance&leave_id=' + leaveId)
        .then(response => response.json())
        .then(data => {
            const checkbox = document.getElementById('filed_advance');
            const statusEl = document.getElementById('status_filed_advance');
            
            if (checkbox && !displayOnly) {
                // Auto-check based on result
                checkbox.checked = data.filed_in_advance;
                ...
                if (label) {
                    ...
                    // Add visual indicator
                    if (data.filed_in_advance) {
                        label.innerHTML += ' <span class="text-success"><i class="fas fa-check-circle"></i> ' + data.days_before + ' days before</span>';
                    } else {
                        label.innerHTML += ' <span class="text-danger"><i class="fas fa-times-circle"></i> ' + data.days_before + ' days before</span>';
                    }
                }
            } else if (statusEl && displayOnly) {
                // Display-only mode
        ...
        .catch(error => {
            console.error('Error checking filed in advance:', error);
        });
}
...
function initializeChecklistListeners() {
    const checkboxes = document.querySelectorAll('#requirementsList input[type="checkbox"]');
...
function updateEligibilityStatus() {
    ...
    // Calculate totals
    const allCheckedCount = checkedCount;
    const allTotalCount = totalCount;
    const allRequirementsMet = checkedCount === totalCount;
...
/**
 * Collect checklist data FROM lc_the modal
 */
