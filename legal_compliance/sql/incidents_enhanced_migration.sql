-- =====================================================
-- ENHANCED INCIDENT MANAGEMENT SYSTEM - MIGRATION
-- With Progressive Discipline Logic
-- Bestlink College of the Philippines
-- =====================================================

-- =====================================================
-- 1. ENHANCE INCIDENTS TABLE - Add progressive discipline columns
-- =====================================================
ALTER TABLE `incidents` 
    ...
    ADD COLUMN `reporter_id` INT(11) DEFAULT NULL AFTER `reporter_name`,
    ADD COLUMN `current_workflow_step` ENUM('submitted', 'under_review', 'validated', 'nte_issued', 'explanation_received', 'hr_evaluation', 'decision_made', 'final_action', 'closed', 'rejected') DEFAULT 'submitted' AFTER `status`,
    ...
    ADD COLUMN `suggested_action` VARCHAR(50) DEFAULT NULL AFTER `created_by`,
    ADD COLUMN `offense_count` INT(11) DEFAULT 0 AFTER `suggested_action`,
    ADD COLUMN `offense_period_months` INT(11) DEFAULT 12 AFTER `offense_count`,
    ADD COLUMN `is_override` TINYINT(1) DEFAULT 0 AFTER `offense_period_months`,
    ADD COLUMN `override_reason` TEXT DEFAULT NULL AFTER `is_override`;

-- Update status to include new workflow statuses
ALTER TABLE `incidents` 
    MODIFY COLUMN `status` ENUM('open', 'under_review', 'in_progress', 'pending_approval', 'resolved', 'rejected', 'escalated', 'closed_no_violation', 'submitted', 'validated', 'nte_issued', 'explanation_received', 'hr_evaluation', 'decision_made', 'final_action', 'closed') DEFAULT 'submitted';
...
CREATE TABLE IF NOT EXISTS `incident_workflow` (
  ...
  `step` ENUM('submitted', 'under_review', 'validated', 'rejected', 'nte_issued', 'explanation_received', 'hr_evaluation', 'decision_made', 'final_action', 'closed') NOT NULL DEFAULT 'submitted',
...
CREATE TABLE IF NOT EXISTS `disciplinary_actions` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `incident_id` INT(11) NOT NULL,
  `action_type` ENUM('verbal_warning', 'written_warning', 'final_written_warning', 'suspension', 'termination', 'case_dismissed', 'counseling', 'probation') NOT NULL,
  ...
  INDEX idx_action_employee (`incident_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `incident_email_log` (
  ...
  `email_type` ENUM('incident_submitted', 'nte_issued', 'nte_reminder', 'explanation_received', 'decision_notice', 'case_closed', 'hr_review', 'incident_rejected') NOT NULL,
...
('validated', 3, 'Validated', 'Incident validated - ready for disciplinary process', NULL, '["issue_nte"]', 1),
('rejected', 4, 'Case Rejected', 'Incident rejected - no action needed', NULL, '["view"]', 1),
('nte_issued', 5, 'Notice to Explain Issued', 'NTE has been issued to employee', 48, '["view_nte", "send_reminder"]', 1),
('explanation_received', 6, 'Explanation Received', 'Employee has submitted explanation', 72, '["evaluate", "request_more_info"]', 1),
('hr_evaluation', 7, 'HR Evaluation', 'HR is evaluating the case', 48, '["decide"]', 1),
('decision_made', 8, 'Decision Made', 'HR has made a decision', NULL, '["view_decision", "finalize"]', 1),
('final_action', 9, 'Final Action', 'Final disciplinary action recorded', 24, '["notify_employee", "close_case"]', 1),
('closed', 10, 'Case Closed', 'Case has been closed', NULL, '["view"]', 1);
...
('progressive_discipline_enabled', '1', 'Enable automatic progressive discipline calculation'),
('offense_tracking_months', '12', 'Default months to track for offense count');

-- =====================================================
-- 10. PROGRESSIVE DISCIPLINE RULES TABLE
-- Define automatic action suggestions based on offense count
-- =====================================================
CREATE TABLE IF NOT EXISTS `progressive_discipline_rules` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `offense_count` INT(11) NOT NULL UNIQUE,
  `action_type` ENUM('verbal_warning', 'written_warning', 'final_written_warning', 'suspension', 'termination') NOT NULL,
...
-- Insert default progressive discipline rules
INSERT INTO `progressive_discipline_rules` (`offense_count`, `action_type`, `action_label`, `description`, `is_active`) VALUES
(1, 'verbal_warning', 'Verbal Warning', 'First offense - verbal warning', 1),
(2, 'written_warning', 'Written Warning', 'Second offense - written warning', 1),
(3, 'suspension', 'Suspension', 'Third offense - suspension', 1),
(4, 'termination', 'Termination', 'Fourth offense or more - termination', 1);
...
-- Calculate offense counts for existing incidents with respondents
UPDATE incidents i
SET i.offense_count = (
    SELECT COUNT(*) 
    FROM incidents 
    WHERE respondent_id = i.respondent_id 
    AND incident_type = i.incident_type
    AND id <= i.id
    AND status IN ('resolved', 'closed')
)
WHERE i.respondent_id IS NOT NULL;

-- Update suggested actions based on offense count
UPDATE incidents i
SET i.suggested_action = (
    CASE 
        WHEN i.offense_count = 1 THEN 'verbal_warning'
        WHEN i.offense_count = 2 THEN 'written_warning'
        WHEN i.offense_count = 3 THEN 'suspension'
        WHEN i.offense_count >= 4 THEN 'termination'
        ELSE 'verbal_warning'
    END
)
WHERE i.offense_count IS NOT NULL;