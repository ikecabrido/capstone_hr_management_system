...
$tables = ['exit_resignations', 'exit_interviews', 'exit_knowledge_transfer_plans', 'exit_knowledge_transfer_items', 'exit_employee_settlements', 'exit_documents', 'exit_payroll_clearances', 'exit_surveys', 'survey_questions', 'survey_responses', 'survey_answers'];
$existingTables = [];
...
CREATE TABLE `exit_payroll_clearances` (
  ...
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `fk_payroll_clearance_settlement` (`settlement_id`),
  KEY `fk_payroll_clearance_employee` (`employee_id`),
  KEY `fk_payroll_clearance_approved_by` (`approved_by`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Constraints
ALTER TABLE `exit_resignations` ADD CONSTRAINT `fk_resignation_approved_by` FOREIGN KEY (`approved_by`) REFERENCES `users` (`id`) ON DELETE SET NULL, ADD CONSTRAINT `fk_resignation_employee` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`) ON DELETE CASCADE, ADD CONSTRAINT `fk_resignation_submitted_by` FOREIGN KEY (`submitted_by`) REFERENCES `users` (`id`) ON DELETE SET NULL;
...
ALTER TABLE `exit_payroll_clearances` ADD CONSTRAINT `fk_payroll_clearance_settlement` FOREIGN KEY (`settlement_id`) REFERENCES `exit_employee_settlements` (`id`) ON DELETE CASCADE, ADD CONSTRAINT `fk_payroll_clearance_employee` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`) ON DELETE CASCADE, ADD CONSTRAINT `fk_payroll_clearance_approved_by` FOREIGN KEY (`approved_by`) REFERENCES `users` (`id`) ON DELETE SET NULL;
