    ...
    switch ($method) {
        ...
        case 'POST':
            if ($action === 'create') {
                ...
                $result = $forumController->createForum($data['title'], $data['description'], $data['category'], $_SESSION['user']['id']);
                if ($result) {
                    echo json_encode(['success' => true, 'message' => 'Forum created successfully']);
                } else {
