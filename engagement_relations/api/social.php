...
try {
    switch ($action) {
        ...
        case 'post':
            ...
            $userId = $_SESSION['user']['id'] ?? $_SESSION['user']['user_id'] ?? null;
            $employeeId = $_SESSION['user']['employee_id'] ?? null;
            if (empty($userId) && empty($employeeId)) jsonResponse(['error' => 'user_id or employee_id required'], 400);
            $authorId = !empty($userId) ? $userId : $employeeId;
            $authorType = !empty($userId) ? 'user' : 'employee';
            $id = $ctrl->createPost($authorId, $data['content'], $authorType);
