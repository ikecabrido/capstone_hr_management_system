...
try {
    switch ($action) {
        ...
        case 'send':
            ...
            $senderUserId = $_SESSION['user']['id'] ?? null;
            if (!$senderUserId) {
                jsonResponse(['error' => 'Current user is not authenticated'], 400);
            }
            $id = $ctrl->sendRecognition($senderUserId, $data['receiver_id'], $data['message'], (int)$data['points']);
        ...
        case 'vote_employee_month':
            ...
            if ($ctrl->hasVotedForAwardHistory((int)$awardHistoryId, (int)$senderUserId)) {
                jsonResponse(['error' => 'You have already voted for this nomination.'], 403);
            }
            ...
            $ctrl->addVotePoints($senderUserId, $nomineeEmployeeId);
