...
<div class="grievance-detail-content">
      ...
      <div class="card card-primary">
        ...
        <div class="card-body">
          ...
          <div class="nav nav-tabs mt-3 mb-3" id="grievance-detail-tabs-<?= (int)($grievance['id'] ?? 0) ?>" role="tablist">
            ...
            <?php if ($showPayrollTab): ?>
              <a class="nav-link" id="grievance-payroll-tab-<?= (int)($grievance['id'] ?? 0) ?>" data-toggle="tab" href="#grievance-payroll-<?= (int)($grievance['id'] ?? 0) ?>" role="tab">Payroll Evidence</a>
            <?php endif; ?>
          </div>
          ...
          <?php if ($isPayrollRelated && (!empty($grievance['payslip_id']) || !empty($grievance['gross_pay']) || !empty($grievance['total_deductions']) || !empty($grievance['net_pay']) || !empty($grievance['payslip_information']))): ?>
            <hr>
          ...
          <?php endif; ?>
                ...
                <?php
                    ...
                    $payslipId = $grievance['payslip_id'] ?? null;
                    // If this is a payroll-related grievance, auto-load the latest payslip for the employee
                    $categoryLabel = strtolower(trim($grievance['category'] ?? ''));
                ...
                <?php if (!empty($payrollContext['tax_tables'])): ?>
                  <hr>
      ...
      <?php if ($showPayrollTab): ?>
        <div class="tab-pane fade" id="grievance-payroll-<?= (int)($grievance['id'] ?? 0) ?>" role="tabpanel">
          <div class="bg-light p-3 rounded">
            <?php if (!empty($payrollContext['payslips']) || !empty($payslipItems) || !empty($adjustments) || !empty($benefits) || !empty($payrollContext['tax_tables'])): ?>
              <?php if (!empty($payrollContext['payslips'])): ?>
                <h5><strong>Payroll Source Summary</strong></h5>
                <ul class="list-unstyled">
                  <?php foreach ($payrollContext['payslips'] as $p): ?>
                    <li><strong>Payslip #<?= (int)($p['payslip_id'] ?? 0) ?>:</strong> Gross <?= '₱' . number_format((float)($p['gross_pay'] ?? 0), 2) ?>, Net <?= '₱' . number_format((float)($p['net_pay'] ?? 0), 2) ?></li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
              <?php if (!empty($payslipItems)): ?>
                <hr>
                <h5><strong>Payslip Items</strong></h5>
                <ul class="list-unstyled">
                  <?php foreach ($payslipItems as $item): ?>
                    <li><?= htmlspecialchars($item['description'] ?? '') ?> — <em><?= '₱' . number_format((float)($item['amount'] ?? 0), 2) ?></em></li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
              <?php if (!empty($adjustments)): ?>
                ...
                <ul class="list-unstyled">
                  <?php foreach ($adjustments as $adj): ?>
                    <li><?= htmlspecialchars($adj['description'] ?? $adj['name'] ?? '') ?> — <em><?= '₱' . number_format((float)($adj['amount'] ?? 0), 2) ?></em></li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
              <?php if (!empty($benefits)): ?>
                ...
                <ul class="list-unstyled">
                  <?php foreach ($benefits as $b): ?>
                    <li>SSS <?= !empty($b['has_sss']) ? 'Enrolled' : 'Not enrolled' ?> • PhilHealth <?= !empty($b['has_philhealth']) ? 'Enrolled' : 'Not enrolled' ?> • Pag-IBIG <?= !empty($b['has_pagibig']) ? 'Enrolled' : 'Not enrolled' ?></li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
              <?php if (!empty($payrollContext['tax_tables'])): ?>
                <hr>
                <h5><strong>Tax Match</strong></h5>
                <ul class="list-unstyled">
                  <?php foreach ($payrollContext['tax_tables'] as $tax): ?>
                    <li>Tax <?= htmlspecialchars($tax['tax_id'] ?? '') ?> — <?= htmlspecialchars(($tax['min_income'] ?? '') . ' - ' . ($tax['max_income'] ?? '')) ?>, Rate <?= htmlspecialchars($tax['tax_rate'] ?? '') ?>%</li>
                  <?php endforeach; ?>
                </ul>
              <?php endif; ?>
            <?php else: ?>
              <p class="mb-0 text-muted">No payroll evidence is linked to this grievance yet.</p>
            <?php endif; ?>
          </div>
        </div>
      <?php endif; ?>
      ...
      <?php if (($grievance['escalation_level'] ?? 1) > 1): ?>
...
<script src="js/grievance_detail.js"></script>
