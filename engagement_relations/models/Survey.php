...
class Survey extends BaseModel
{
    ...
    {
        $sql = 'INSERT INTO eer_surveys (title, created_by, date_created) VALUES (:title, :created_by, NOW())';
        $params = [
            'title' => $title,
    ...
    {
        return $this->execute('SELECT * FROM eer_surveys ORDER BY date_created DESC')->fetchAll();
    }
    ...
    {
        $survey = $this->execute('SELECT * FROM eer_surveys WHERE eer_survey_id = :id', ['id' => $id])->fetch();

        if (!$survey) {
            return null;
        }

        $survey['questions'] = $this->execute('SELECT * FROM eer_survey_questions WHERE survey_id = :id', ['id' => $id])->fetchAll();
        return $survey;
    }
    ...
    public function deleteSurvey($id)
    {
        ...
        return $this->execute('DELETE FROM eer_surveys WHERE eer_survey_id = :id', ['id' => $id])->rowCount() > 0;
    }
}
