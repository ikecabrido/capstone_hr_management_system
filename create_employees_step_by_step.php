...
try {
    ...
    echo "Creating employees table (step by step)...\n";

    // Step 1: Create table without constraints
    echo "Step 1: Creating basic table structure...\n";
    $createTableSQL = "
    ...
    echo "Step 2: Adding primary key...\n";
    $db->exec("ALTER TABLE `employees` ADD PRIMARY KEY (`employee_id`)");
    echo "Primary key added\n";

    // Step 3: Add auto increment
    echo "Step 3: Adding auto increment...\n";
    ...
    // Step 5: Convert to InnoDB and add foreign key constraints
    echo "Step 5: Converting to InnoDB...\n";
    $db->exec("ALTER TABLE `employees` ENGINE = InnoDB");
    echo "Table converted to InnoDB\n";
    ...
    // Show sample data
    echo "\nSample records:\n";
    $sampleStmt = $db->query('SELECT employee_id, employee_no, full_name, department FROM employees LIMIT 5');
    while ($row = $sampleStmt->fetch(PDO::FETCH_ASSOC)) {
        echo "- ID: " . $row['employee_id'] . ", No: '" . $row['employee_no'] . "', Name: " . $row['full_name'] . " (" . $row['department'] . ")\n";
    }
