...
class PolicyWorkflowController
{
    ...
    /**
     * Delete policy (only Draft status)
     */
    public function deletePolicy($id)
    {
        $stmt = $this->db->prepare("DELETE FROM policies WHERE id = ? AND status = 'Draft'");
    ...
    {
        try {
            // Check if policies table exists and has status column
            $this->db->query("SELECT status FROM policies LIMIT 1");
        } catch (Exception $e) {
            // Table doesn't exist or no status column, return empty array
            return [];
        }
