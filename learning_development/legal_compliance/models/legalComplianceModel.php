...
class LegalComplianceModel
{
    ...
    // Get filtered employees by compliance status
    public function getFilteredEmployees($filter)
    {
        // If filter is 'all', return all employees
        if ($filter === 'all') {
            return $this->getEmployeesWithScores();
        }
        
        // Map filter values to compliance status
        $statusMap = [
            'compliant' => 'compliant',
            'at-risk' => 'at_risk',
        ...
        // Build query based on compliance score thresholds
        if ($status === 'compliant') {
            // Score >= 80
            $stmt = $this->db->query("
                SELECT e.*, 
                ...
                FROM employees e
                LEFT JOIN compliance_summary cs ON e.id = cs.employee_id
                WHERE e.status = 'Active' AND (cs.overall_score >= 80 OR cs.overall_score IS NULL)
            ...
            $stmt = $this->db->query("
                ...
                FROM employees e
                LEFT JOIN compliance_summary cs ON e.id = cs.employee_id
                WHERE e.status = 'Active' AND cs.overall_score >= 60 AND cs.overall_score < 80
                ORDER BY cs.overall_score ASC
            ");
        } else {
            // Score < 60
            $stmt = $this->db->query("
                ...
                FROM employees e
                LEFT JOIN compliance_summary cs ON e.id = cs.employee_id
                WHERE e.status = 'Active' AND cs.overall_score < 60
                ORDER BY cs.overall_score ASC, cs.critical_issues DESC
            ");
        }
