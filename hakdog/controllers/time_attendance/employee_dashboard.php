<?php
/**
 * Employee Personal Dashboard - Time & Attendance
 * Shows individual employee attendance stats, leave balance, and performance
 * Uses root-level database connector from auth/database.php
 */
...
try {
    // Use root-level database connector with absolute paths
    $rootDir = dirname(__DIR__, 3); // Go up 3 levels to root
    require_once $rootDir . '/auth/database.php';
    require_once $rootDir . '/auth/auth.php';
    
    // Load employee portal dependencies
    require_once dirname(__DIR__) . '/app/controllers/AttendanceController.php';
    require_once dirname(__DIR__) . '/app/models/Employee.php';
    require_once dirname(__DIR__) . '/app/models/Attendance.php';
    require_once dirname(__DIR__) . '/app/models/Leave.php';
    require_once dirname(__DIR__) . '/app/models/EmployeeShift.php';
    require_once dirname(__DIR__) . '/app/models/Shift.php';
    require_once dirname(__DIR__) . '/app/helpers/Helper.php';
    require_once dirname(__DIR__) . '/app/core/Session.php';
    
    Session::start();

    // Check authentication using root auth
    $auth = new Auth();
    if (!$auth->check()) {
    ...
    // Get user ID from session (set by root auth system)
    ...
    // Get database connection from root connector (singleton)
